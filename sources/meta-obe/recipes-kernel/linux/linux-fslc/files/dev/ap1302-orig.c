...
static int ap1302_parse_of(struct ap1302_device *ap1302)
{
	...
	if (IS_ERR(ap1302->reset_gpio)) {
		if (PTR_ERR(ap1302->reset_gpio) == -ENOENT)
		{
			dev_warn(ap1302->dev,"reset gpio not defined in devicetree - externally powered?\n");
			ap1302->reset_gpio = NULL;
		}
		else
		{
	...
	if (IS_ERR(ap1302->standby_gpio)) {
		if (PTR_ERR(ap1302->standby_gpio) == -ENOENT)
		{
			dev_warn(ap1302->dev,"standby gpio not defined in devicetree - externally powered?\n");
			ap1302->standby_gpio = NULL;
		}
		else
		{
...
static int ap1302_probe(struct i2c_client *client)
{
	...
	dev_info(ap1302->dev, "AP1302 Driver\n");
